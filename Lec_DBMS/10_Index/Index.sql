...
-- 사원의 인덱스
SELECT EMPNO, ROWID FROM T_EMP;  -- ROWID, ROW INSERT 될 때마다 부여됨, SELECT 시 기본적으로 참조 

SELECT EMPNO, ROWID FROM T_EMP ORDER BY EMPNO DESC; -- ROWID 도 내림차순으로 된다. 

SELECT ENAME, ROWID -- ROWID 순으로 나옴.  
FROM T_EMP
ORDER BY ENAME ASC
;
...
-- 인덱스 생성 구문   (시험)))))) )  특정테이블에 특정 컬럼에 오름차순에 적용하는 인덱스를 무슨 이름으로 만드세요 .
...
CREATE UNIQUE INDEX idx_dept2_dname ON t_professor(dname ASC)

SELECT * FROM T_DEPT2 ;
CREATE UNIQUE INDEX idx_dept2_dname ON t_dept2(dname);  -- DDL 명령어로 만듦.   DNAME 에 유니크를 걸어놓으면  검색 조건이 향상될 수 있음. 

SELECT DNAME FROM T_DEPT2 ;

INSERT INTO T_DEPT2 VALUES(9100, '임시매장', 1006, '서울지사');  

INSERT INTO T_DEPT2 VALUES(9101, '임시매장', 1006, '부산지사');-- ORA-00001: unique constraint (SCOTT0316.IDX_DEPT2_DNAME) violated 
-- DNAME 컬럼에 유니크를 걸어놨기 때문에 에러뜬다. 
 ...
 * WHERE sal + 100 = 200.. ??
 * 
 * 그러면 인덱스를 생성할때 저 형태로 , 즉 sal + 100  연산에다가 인덱스를 생성하면 됨.
 * 이를 '함수 기반 인덱스' 라 함.
 * 
 */
...
ON t_professor(pay DESC);  -- 계좌조회 나 게시판 날짜순으로  할때 인덱스가 성능에   
