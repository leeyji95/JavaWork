package com.lec.java.string03stringbuffer;
...
public class String03Main {
	...
	public static void main(String[] args) {
		...
		System.out.println(str1);  // 원본의 변화는 없다   -> str1 이 실제 변경되지 않는다
		str1 = str1.concat("Java");  // 덮어씀  
		System.out.println(str1);
		
		if(str1 == str2) {  // 주소 비교(레퍼런스타입이므로)
			System.out.println("=== 동일 참조");
		} else {
			System.out.println("== 다른 참조");
		}
		...
		if(buff1 == buff2) {  // 주소 비교(레퍼런스타입이므로)
			System.out.println("== 동일 참조");
		}else {
			System.out.println("== 다른 참조");
		}
		...
		// StringBuffer:
		//  Java ver 1.0부터 있던 클래스
		//  쓰레드에 안전(thread-safe), 속도가 느림  -----> 신호등 빨간불 들어오면 멈추나
		
		// StringBuilder:
		// Java 5부터 생긴 클래스
		// 쓰레드에 안전하지 않다(thread-unsafe), 속도가 빠름.  ------> 얘는 안 멈춘다... 
