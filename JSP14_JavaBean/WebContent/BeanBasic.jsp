...
<!-- person 형 객체 생성한 의미이다. -->
<jsp:useBean id="person1" class="com.lec.beans.Person" scope="page" />
<!-- 
    Person person1 = new com.lec.beans.Person();  // 기본 생성자 호출 역할 !  <-- 내부적으로 이런 동작을 했다고 생각하면 된다.
    id 가 마치 변수 이름과 같음	
    request.setAttribute("person1", person1); // 좀 더 정확히는 이 코드도 수행한다! 		
		      -->
...
<!-- form 에서 넘어온 값을 bean객체가 바로 받을 수 있다.
...
<jsp:setProperty name="person4" property="*"/>  <!-- <-- 이거 지금 뭐하는 거다?  parameter 를 일괄로 받아오기    -->
...
<body>
	...
	<!-- 내부적으로 다음과 같은 동작을 수행한다.	
		person1.setName("고길동") ;   놀라운건 프라퍼티로 고길동 하면 알아서 setName을 호출한다.
		id값을 setProperty 에서는 name 값으로 받아서 가져온다... 
		 -->
	...
	<br> 아이디:
	<jsp:getProperty name="person1" property="id" />
	<br> 성별:
	<jsp:getProperty name="person1" property="gender" />
	<br>
	<!--	내부적으로 person1.getName()  동작하여 나옴.   -->
