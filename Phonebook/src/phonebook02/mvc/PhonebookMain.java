...
// 사용자 입력 받아서 전화번호부에 데이터 저장해 나갈것. 
	// 멤버변수
	private Scanner sc;
	private PhonebookManager pbCtroller; // CONTRㅐLLER 객체  
...
//--------------------------------------------------------	
	...
	public void init()	{
		...
		pbCtroller = PhonebookManager.getInstance();  // t
	}
...
//--------------------------------------------------------	
	...
	public void insertPhoneBook() {
		...
		// CONTROLLER  에 연결
		// view 에서는 인터페이스에 있는 메소드를 사용한다. 그것만 사용한다. 
		// 가교역할하는 인터페이스.  컨트롤러와 뷰 사이의 가교 역할하는 인터페이스 
		int result = pbCtroller.insert(name, phoneNum, memo);
		if(result == PhonebookController.QUERY_FAIL) {  // static 이니까 클래스이름 쩜.
	...
	public void listPhonebook() {
		// CONTROLLER 연결
		PhonebookModel[] data = pbCtroller.selectAll();
	...
	public void updatePhonebook() {
		...
		if(pbCtroller.selectByUid(uid) == null) {
		...
		// CONTROLLER 연결
		int result = pbCtroller.updateByUid(uid, name, phoneNum, memo); // 이 값을 정수값으로 결과 받고,
	...
	public void deletePhonebook() {
		...
		int result = pbCtroller.deleteByUid(uid);
		if(result == PhonebookController.QUERY_FAIL) {
